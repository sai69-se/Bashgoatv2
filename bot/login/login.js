...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        // ——————————————————— LOAD DATA ——————————————————— //
                        const { threadModel, userModel, globalModel, threadsData, usersData, globalData, sequelize } = await require(process.env.NODE_ENV === 'development' ? "./loadData.dev.js" : "./loadData.js")(api, createLine);
                        // ————————————————— CUSTOM SCRIPTS ————————————————— //
                        await require("../custom.js")({ api, threadModel, userModel, globalModel, threadsData, usersData, globalData, getText });
                        // —————————————————— LOAD SCRIPTS —————————————————— //
                        await require(process.env.NODE_ENV === 'development' ? "./loadScripts.dev.js" : "./loadScripts.js")(api, threadModel, userModel, globalModel, threadsData, usersData, globalData, createLine);
                        ...
                        async function callBackListen(error, event) {
                                ...
                                const handlerAction = require("../handler/handlerAction.js")(api, threadModel, userModel, globalModel, usersData, threadsData, globalData);
                        ...
                        // ——————————————————— UPTIME ——————————————————— //
                        if (global.GoatBot.config.serverUptime.enable == true && !global.serverUptimeRunning) {
                                ...
                                const PORT = (!isNaN(global.GoatBot.config.serverUptime.port) && global.GoatBot.config.serverUptime.port) || 3001;
